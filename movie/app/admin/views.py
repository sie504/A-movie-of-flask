...
def admin_auth(f):
    ...
    def decoreted_function(*args,**kwargs):
        ...
        auth_list = Auth.query.all()  #权限列表
        urls = [v.url for v in auth_list for val in auths if val == v.id] #列表推导
        rule = request.url_rule
        #request是flask的一个对象，很很多的属性，比如这个request.url_rule，就是索取url除去域名后的信息
        print(urls)  #权限地址，url
        # '/admin/movie/add/', '/admin/preview/add/', '/admin/tag/add/']
        print(rule)
        #/admin/
...
def tag_list(page=None):
    ...
    page_data = Tag.query.order_by(
        Tag.addtime.desc()
    ).paginate(page=page, per_page=10)  # paginate分页操作
...
def movie_edit(id=None):
    ...
    if request.method == "GET":
        form.info.data = movie.info  # 将内容显示到
...
# 删除评论成功
@admin.route("/moviecol/del/<int:page>/", methods=["GET"])
@admin_login_req
@admin_auth
...
def role_add():
    ...
    if form.validate_on_submit():
        ...
        role = Role(
            name = data["name"],
            auths = ",".join(map(lambda v:str(v),data["auths"]))  #列表--字符串
        )
        db.session.add(role)
        db.session.commit()
        flash("添加角色成功","ok")
    return render_template("admin/role_add.html",form=form)  #后端和前端的交互，要把其中的内容传递到前端
