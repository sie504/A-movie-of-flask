...
class User(db.Model):
    ...
    userlogs = db.relationship("Userlog", backref='user')  # 会员日志外联关系
    comments = db.relationship("Comment", backref='user')  # 评论外联关系
    moviecols = db.relationship("Moviecol", backref='user')  # 电影收藏外联关系
...
class Movie(db.Model):
    ...
    playnum = db.Column(db.BigInteger)  # 评论
    commentnum = db.Column(db.BigInteger)  # 评论
    tag_id = db.Column(db.Integer, db.ForeignKey('tag.id'))  # 所属标签
    area = db.Column(db.String(100))  # 上映地区
    release_time = db.Column(db.String(100))  # 上映时间
    length = db.Column(db.String(100))  # 播放时间
    addtime = db.Column(db.DateTime, index=True, default=datetime.utcnow)  # 添加时间
    comments = db.relationship("Comment", backref='movie')  # 评论外联关系
    moviecols = db.relationship("Moviecol", backref='movie')  # 电影收藏外联关系
...
class Role(db.Model):
    __tablename__ = "role"
    id = db.Column(db.Integer, primary_key=True)  # 编号
    name = db.Column(db.String(600), unique=True)  # 名称
...
class Admin(db.Model):
    ...
    is_super = db.Column(db.SmallInteger)  # 是否超级用户
